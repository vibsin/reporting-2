...
class BgsController extends Zend_Controller_Action {
    public $data;
    ...
    public function indexAction() {
        ...
        if($this->getRequest()->isPost()) {
            ...
            if($isvalidPost === true) {
                ...
                if($isSummarize == 'on') {
                    ...
                    if($status === true) {
                        ...
                        $this->view->summarizedData = $this->getDateItems($posts['bgs_summarize_intervals_of'][0],$posts);
            ...
            $this->renderScript('bgs/index.phtml');
        }
    ...
    protected function validateDates($posts) {
    	...
    	//unsubscribe date
    	if(!empty($posts['bgs_filter_lead_date_from']) &&
                !empty($posts['bgs_filter_lead_date_to'])) {
            $from = strtotime($posts['bgs_filter_lead_date_from']);
            $to = strtotime($posts['bgs_filter_lead_date_to']);
    ...
    public function validateSummarize($posts) {
    	//print_r($posts['bgs_summarize_for_date']);  exit;
        if(!empty($posts['bgs_summarize_by_count_of']) &&
                        !empty($posts['bgs_summarize_for_date']) &&
                        !empty($posts['bgs_summarize_intervals_of'])) {
                        	
                        	
        	//now check if the dates are not blank and if present should be proper dates

        	switch($posts['bgs_summarize_for_date'][0]) {
        		case 'lead_item_date_tdt':
        			if(!$this->validateDate($posts['bgs_filter_lead_date_from']) || 
        				!$this->validateDate($posts['bgs_filter_lead_date_to'])) {
...
//            else if ($caption == 'Unsubscribe Date') $obj3->fl = 'unsubscribe_date';
    ...
    public function getDateItems($interval,$posts) {
        $summarizedData = "";
        $dates = $this->getDatesForThisPeriod($posts['bgs_summarize_for_date'][0], $posts);
        $caption = '';

        if($posts['bgs_summarize_for_date'][0] == 'lead_item_date_tdt') {
            $caption = 'Lead Date';
        } 
        ...
        switch($interval) {
            case 'daily':
                ...
                while($current <= $toInMicro) {
                    //echo $current;
                    $obj3 = new Model_BgsSolr($posts);
                    $obj3->fl = "lead_item_date_tdt";
//                    if($caption == 'Lead Date') $obj3->fl = 'creation_date';
//                    else if ($caption == 'Unsubscribe Date') $obj3->fl = 'unsubscribe_date';
...
//                    else if ($caption == 'Unsubscribe Date') $obj3->fl = 'unsubscribe_date';
    ...
    public function getDatesForThisPeriod($filterDate,$posts) {
        ...
        switch($filterDate) {
            case 'lead_item_date_tdt':
                return array('from' => $posts['bgs_filter_lead_date_from'], 'to' => $posts['bgs_filter_lead_date_to']);
            break;
