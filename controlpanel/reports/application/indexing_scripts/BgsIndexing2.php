...
class Bgsindexing extends IndexingAbstract {
    ...
    protected $indexingUrl = SOLR_BGS_INDEXING_URL; //"http://172.16.1.67:8983/solr/bgs_new/";
    ...
    protected function initBuildingData() {
        foreach(self::$data as $key => $val) {
            ...
            self::$dataToIndex[self::$counter]['lead_item_type_s'] = $this->parseItemType(trim($val['item_type'])); //done Reply/Lead
            self::$dataToIndex[self::$counter]['lead_item_id_l'] = trim($val['item_id']); //done reply id/ad id
            self::$dataToIndex[self::$counter]['lead_item_date_tdt'] = $this->convertToUTC(trim($val['created_time'])); //done
            self::$dataToIndex[self::$counter]['bgs_poster_email_s'] = trim($val['email']); //BGS user email
            self::$dataToIndex[self::$counter]['bgs_poster_id_l'] = trim($val['user_id']); //BGS user id
            self::$dataToIndex[self::$counter]['bgs_poster_mobile_s'] = $smbDetails['mobile']; //BGS user id
            self::$dataToIndex[self::$counter]['action_item_id_l'] = trim($val['action_id']); //if lead type is reply then ad else alert
            ...
            if(trim($val['item_type']) == "1") { //reply
                ...
                $adsArr = array();
                    ...
                    self::$dataToIndex[self::$counter]['action_item_city_id_l'] = trim($adsArr['city_id']); //done
                    self::$dataToIndex[self::$counter]['action_item_city_name_s'] = trim($adsArr['city_name']); //done
                    self::$dataToIndex[self::$counter]['action_item_category_id_l'] = trim($adsArr['metacategory_id']); //done
                    self::$dataToIndex[self::$counter]['action_item_category_name_s'] = trim($adsArr['metacategory_name']); //done
                    self::$dataToIndex[self::$counter]['action_item_subcategory_id_l'] = trim($adsArr['subcategory_id']); //done
                    self::$dataToIndex[self::$counter]['action_item_subcategory_name_s'] = substr_replace(trim($adsArr['subcategory_name']),'',0,strpos(trim($adsArr['subcategory_name']), ',')+1); //done
                    if($adsArr['free_premium_type'] == "Free") {
                        self::$dataToIndex[self::$counter]['action_item_style_s'] = trim($adsArr['free_premium_type']); //done
                    } else {
                        self::$dataToIndex[self::$counter]['action_item_style_s'] = trim($adsArr['premium_ad_type']); //done
                    }
                    self::$dataToIndex[self::$counter]['action_item_global_metacategory_id_l'] = trim($adsArr['global_metacategory_id']); //done
                    self::$dataToIndex[self::$counter]['action_item_global_subcategory_id_l'] = trim($adsArr['global_subcategory_id']); //done
                    
                    self::$dataToIndex[self::$counter]['action_item_title_t'] = trim($adsArr['ad_title']); //done
                    self::$dataToIndex[self::$counter]['action_item_desc_t'] = trim($adsArr['ad_description']); //done
                ...
                if (!empty($premData) && $premData->response->numFound > 0) {
                    ...
                    if(isset($premArr['premiumads_pack_id']) && !empty($premArr['premiumads_pack_id']))  {
                            self::$dataToIndex[self::$counter]['action_item_pack_id_l'] = trim($premArr['premiumads_pack_id']); //done
                    }

                    if(isset($premArr['premiumads_vd_status']) && !empty($premArr['premiumads_vd_status']))  {
                            self::$dataToIndex[self::$counter]['action_item_pack_status_s'] = trim($premArr['premiumads_vd_status']); //done
                    }
                    if(isset($premArr['premiumads_vd_ro_name']) && !empty($premArr['premiumads_vd_ro_name']))  {
                        self::$dataToIndex[self::$counter]['action_item_ro_name_s'] = trim($premArr['premiumads_vd_ro_name']); //done
                    }
                    if(isset($premArr['premiumads_order_id']) && !empty($premArr['premiumads_order_id']))  {
                        self::$dataToIndex[self::$counter]['action_item_pack_order_id_s'] = trim($premArr['premiumads_order_id']); //done
                    }
                    if(isset($premArr['premiumads_vdu_area_id']) && !empty($premArr['premiumads_vdu_area_id']))  {
                        self::$dataToIndex[self::$counter]['action_item_pack_city_id_l'] = trim($premArr['premiumads_vdu_area_id']); //done
                        self::$dataToIndex[self::$counter]['action_item_pack_city_name_s'] = $this->getCityName($premArr['premiumads_vdu_area_id']); //done
                    }
                ...
                $replyArr = array();
                    ...
                    self::$dataToIndex[self::$counter]['lead_item_email_s'] = $replyArr["rpl_email"]; 
                    self::$dataToIndex[self::$counter]['lead_item_mobile_s'] = $replyArr["rpl_mobile"]; 
                    self::$dataToIndex[self::$counter]['lead_item_desc_t'] = $replyArr["rpl_content"]; 
                }
                        ...
                        foreach ($story as $k => $v) {
                            if($k == "subcatid") {
                                ...
                                if($num == 0 || $v == 0) { //error
                                    ...
                                    self::$dataToIndex[self::$counter]['action_item_category_id_l'] = $metacatId;
                                    self::$dataToIndex[self::$counter]['action_item_category_name_s'] =  $metacatName;
                                    self::$dataToIndex[self::$counter]['action_item_subcategory_id_l'] = $subcatId;
                                    self::$dataToIndex[self::$counter]['action_item_subcategory_name_s'] = $subcatName; 
                                    self::$dataToIndex[self::$counter]['action_item_global_metacategory_id_l'] = $globalMetacatId;
                                    self::$dataToIndex[self::$counter]['action_item_global_subcategory_id_l'] = $globalSubcatId;
                            ...
                            if($k=="category") {
                                ...
                                self::$dataToIndex[self::$counter]['action_item_city_id_l'] = $match[1]; //done
                                self::$dataToIndex[self::$counter]['action_item_city_name_s'] = $this->getCityName($match[1]); //done

                            }
//                            //mail
//                            if($k=="userinfo") {
//                                preg_match('/mobile\:(\d+)/', $value, $match); 
//                                self::$dataToIndex[self::$counter]['action_item_mobile_s'] = $match[1]; //done
//                            }
                    ...
                    if(is_array($premAlert)) {
                        ...
                        self::$dataToIndex[self::$counter]['action_item_pack_id_l'] = trim($premAlert['pack_id']); //done
                        self::$dataToIndex[self::$counter]['action_item_pack_status_s'] = $this->parseVduStatus(trim($premAlert['pack_status'])); //done
                        self::$dataToIndex[self::$counter]['action_item_ro_name_s'] = trim($premAlert['ro_name']); //done
                        self::$dataToIndex[self::$counter]['action_item_pack_order_id_s'] = trim($premAlert['order_id']); //done
                        self::$dataToIndex[self::$counter]['action_item_pack_city_id_l'] = trim($premAlert['pack_city_id']); //done
			self::$dataToIndex[self::$counter]['action_item_pack_city_name_s'] = $this->getCityName(trim($premAlert['pack_city_id'])); //done
                    ...
                    if (!empty($adsData) && $adsData->response->numFound > 0) {
                        ...
                        self::$dataToIndex[self::$counter]['lead_item_email_s'] = $adsArr["poster_email"];
                        self::$dataToIndex[self::$counter]['lead_item_title_t'] = trim($adsArr['ad_title']); //done
                        self::$dataToIndex[self::$counter]['lead_item_desc_t'] = trim($adsArr['ad_description']); //done
                        self::$dataToIndex[self::$counter]['lead_item_mobile_s'] = trim($adsArr['poster_mobile']); //done
                        
                        if($adsArr['free_premium_type'] == "Free") {
                            self::$dataToIndex[self::$counter]['lead_item_style_s'] = trim($adsArr['free_premium_type']); //done
                        } else {
                            self::$dataToIndex[self::$counter]['lead_item_style_s'] = trim($adsArr['premium_ad_type']); //done
                        }
                    }
    ...
    private function parseItemType($type) {
        if($type == "1") return "Reply"; else return "Ad"; 
    }
    
    private function getSmbDetails($email) {

        $sql = 'select * from smb_details where email = "' . $email . '" LIMIT 1';
        $objStmt = new Zend_Db_Statement_Pdo(Zend_Registry::get('dbconnection'), $sql);
        $objStmt->execute();
        $items = $objStmt->fetch();
