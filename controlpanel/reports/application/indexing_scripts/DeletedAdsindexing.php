...
class DeletedAdsindexing extends IndexingAbstract {
    ...
    public function init($args) {
        ...
        //print_r($args); exit;
            ...
            case 'DATE_RANGE_ARCHIVE':
                    $this->isIncrementalIndexing = false;
                    $this->dbConnection = Zend_Registry::get("archivedb");
                    $past = strtotime($args[2]);
                    $now = strtotime($args[3]);
...
//self::$dataToIndex[self::$counter]['no_of_visitors'] = $this->getNoOfVisitors($val['tpc_id']);
...
//            } else {
            self::$dataToIndex[self::$counter]['ad_delete_date'] = $val['tpc_lastupdated'];
           // }
            ...
            self::$dataToIndex[self::$counter]['no_of_replies'] = 0; //$this->getReplyCountForAd($val['tpc_id']); //done
            ...
            */
            ...
            if($this->commandArgs[1] == "DEL_ADID") {
                self::$dataToIndex[self::$counter]["db_table_txt"] = $this->dbTableName;
            } else if($this->commandArgs[1] == "ARCHIVE_ADID") {
                self::$dataToIndex[self::$counter]["db_table_txt"] = "archive_".$this->dbTableName;
            } else {
                self::$dataToIndex[self::$counter]["db_table_txt"] = "NA";
            }
            
            if($this->isIncrementalIndexing == true) {
//                //update user count
                  self::$dusersToUpdate[] = $val['tpc_uid'];
//                //update the reply core with updated ads
                  self::$dreplyWithAdsToUpdate[] = $val['tpc_id'];
            }
            self::$counter++;
