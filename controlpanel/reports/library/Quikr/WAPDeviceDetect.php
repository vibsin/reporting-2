...
class Quikr_WAPDeviceDetect {

    public $user_agent;
    public $accept;
    public $wapDevice;

    public function __construct($user_agent='',$accept='') {
        if(!empty($user_agent)) $this->user_agent = $user_agent;
        else $this->user_agent = $_SERVER['HTTP_USER_AGENT'];

        if(!empty($accept)) $this->accept = $accept;
        else $this->accept = $_SERVER['HTTP_ACCEPT'];
